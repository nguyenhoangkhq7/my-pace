...
/** Left border accent + badge styling per urgency */
...
function contextBadgeStyle(colorCode: string): React.CSSProperties {
...
export function KanbanCard({ task }: KanbanCardProps) {
  ...
  const hasFooter = Boolean(task.context || task.estimatedMinutes != null);
      ...
      <div className="px-3 py-2.5">
        ...
        <div className="flex items-start gap-2">
          ...
          <button
            ...
            className={cn(
              "mt-[3px] shrink-0 transition-all duration-150 hover:scale-110 active:scale-95",
          ...
          {editing ? (
            ...
            <p
              className={cn(
                "flex-1 text-sm font-medium leading-5 text-slate-100 break-words",
          ...
          {!editing && (
            <button
              ...
              className={cn(
                "mt-[3px] shrink-0 text-slate-600 hover:text-slate-300",
        ...
        {hasFooter && (
          <div
            className={cn(
              "mt-2 pl-[23px] flex items-center gap-2",
          ...
          >
            ...
            {task.context && (
              <span
                className={cn(
                  ...
                  !task.context.colorCode &&
                  "border-slate-600/40 bg-slate-700/40 text-slate-400"
                )}
                style={
                  task.context.colorCode
                    ? contextBadgeStyle(task.context.colorCode)
                    : undefined
                }
              >
                {task.context.name}
            ...
            {/* Spacer */}
            {task.context && task.estimatedMinutes != null && (
